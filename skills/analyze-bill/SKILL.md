...
- 列出所有出现的商户名称作为参考
- 生成空的 `target_aliases` 供用户手动填充

### 请用户校对配置

生成后，向用户汇报自动检测结果：
- 交易状态：成功 X 种、失败 Y 种、未知 Z 种
- 如果有标记为 `unknown` 的状态或方向，**必须**提醒用户手动确认
- 告知用户可以在配置文件中编辑 `target_aliases` 来归并同一商户的不同名称

**请用户确认配置是否正确。** 尤其关注：
- 是否有状态被错误分类？（如"退款成功"应该算成功还是排除？）
- 是否需要添加商户别名？（如"杭州青奇"归入"哈啰"）

用户可以直接编辑 `/tmp/analyze_bill_config.json`，然后告知你是否已确认。确认后进入交互模式。
